...
# I will find 'if msg_clean == "divers":' and the following code until 'if msg_clean in ["talon", "talons"]:'
new_block = """        if msg_clean == "divers":
...
start_idx = content.find('        if msg_clean == "divers":')
end_idx = content.find('        if msg_clean in ["talon", "talons"]:')

content = content[:start_idx] + new_block + "\n" + content[end_idx:]

with open(filepath, 'w', encoding='utf-8') as f:
    f.write(content)
print("Cleanly fixed API")
