...
class WhatsAppStockController(http.Controller):
    ...
    def whatsapp_stock(self, **kwargs):
        ...
        fast_stock = request.env['casa.stock.stock'].sudo().search([
        ...
        products = request.env['casa.product'].sudo().search([('article_id', 'in', articles.ids)])
        
        # Check stock globally for these products (Strictly positive stock)
        stock_records = request.env['casa.stock.stock'].sudo().search([
        ...
        if len(articles_in_stock) == 0:
            ...
            pdf_content, _ = report_action._render_qweb_pdf('casa_stock.action_report_casa_stock_product', res_ids=stock_for_pdf.ids)
    ...
    def _send_global_report(self):
        ...
        dummy_record = request.env['casa.stock.stock'].sudo().search([('quantity', '>', 0)], limit=1)
        
        if not dummy_record:
            return {'status': 'not_found', 'message': "Désolé, il n'y a actuellement aucun article en stock pour générer le rapport général."}
        
        pdf_content, _ = request.env['ir.actions.report'].sudo()._render_qweb_pdf('casa_stock.action_report_casa_stock_general', res_ids=dummy_record.ids)
        return {
            'status': 'success',
