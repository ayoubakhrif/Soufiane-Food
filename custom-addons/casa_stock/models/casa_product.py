...
class CasaProduct(models.Model):
    ...
    def action_generate_product_report(self):
        self.ensure_one()
        stock_records = self.env['casa.stock.stock'].search([
            ('product_id', '=', self.id),
            ('quantity', '>', 0)
        ])
        ...
        return self.env.ref('casa_stock.action_report_casa_stock_product').report_action(stock_records)

    def action_generate_general_report(self):
        return self.env.ref('casa_stock.action_report_casa_stock_general').report_action(None)
