...
class CasaStockDiscount(models.Model):
    _name = 'casa_hanane.stock.discount'
    _description = 'Réduction Commerciale (Hanane)'
    _order = 'date desc, id desc'

    name = fields.Char(string='Référence', readonly=True, default='/')

    order_id = fields.Many2one(
        'casa_hanane.stock.order', string='Commande', required=True,
        domain="[('state', '=', 'done')]",
    )
    ...
    line_ids = fields.One2many(
        'casa_hanane.stock.discount.line', 'discount_id', string='Lignes de Réduction',
    )
    ...
    def create(self, vals):
        if vals.get('name', '/') == '/':
            vals['name'] = self.env['ir.sequence'].next_by_code('casa_hanane.stock.discount') or '/'
...
class CasaStockDiscountLine(models.Model):
    _name = 'casa_hanane.stock.discount.line'
    _description = 'Ligne de Réduction Commerciale (Hanane)'

    discount_id = fields.Many2one(
        'casa_hanane.stock.discount', string='Réduction',
        required=True, ondelete='cascade',
    )
    client_id = fields.Many2one('casa_hanane.client', related='discount_id.client_id', store=True, string='Client')
    state = fields.Selection(related='discount_id.state', string='État', store=True)

    exit_id = fields.Many2one('casa_hanane.stock.exit', string='Sortie', required=True, readonly=True)

    # Related fields from exit (readonly)
    product_id = fields.Many2one('casa_hanane.product', string='Produit', related='exit_id.product_id', store=True, readonly=True)
