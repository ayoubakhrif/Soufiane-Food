...
class CasaStockEntry(models.Model):
    _name = 'casa_hanane.stock.entry'
    _description = 'Entrée Stock Casa (Hanane)'
    ...
    provider_id = fields.Many2one('casa_hanane.provider', string='Fournisseur', tracking=True)
    ...
    def create(self, vals):
        if vals.get('name', '/') == '/':
            vals['name'] = self.env['ir.sequence'].next_by_code('casa_hanane.stock.entry') or '/'
    ...
    def _get_current_stock_qty(self, rec, price=None):
        ...
        res = self.env['casa_hanane.stock.move'].read_group(domain, ['qty'], [])
