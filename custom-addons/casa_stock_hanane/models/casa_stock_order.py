...
class CasaStockOrder(models.Model):
    _name = 'casa_hanane.stock.order'
    _description = 'Commande de Stock Casa (Hanane)'
    _order = 'date desc, id desc'
    _inherit = ['mail.thread', 'mail.activity.mixin']

    name = fields.Char(string='Référence', readonly=True, default='/')
    client_id = fields.Many2one('casa_hanane.client', string='Client', required=True)
        ...
        if 'order_line_ids' in vals:
            ...
            for command in vals['order_line_ids']:
                if command[0] == 0: # Create command
                    line_vals = command[2]
                    product = self.env['casa_hanane.product'].browse(line_vals.get('product_id'))
                    if not product and 'stock_id' in line_vals:
                        product = self.env['casa_hanane.stock.stock'].browse(line_vals['stock_id']).product_id
            ...
            if 'order_line_ids' in vals:
                for command in vals['order_line_ids']:
                    if command[0] == 0: # Create
                        ...
                        line_id = command[1]
                        line_rec = self.env['casa_hanane.stock.order.line'].browse(line_id)
    ...
    def action_confirm(self):
        for order in self:
            ...
            generated_exits = self.env['casa_hanane.stock.exit']
            ...
            for line in order.order_line_ids:
                ...
                new_exit = self.env['casa_hanane.stock.exit'].create(exit_vals)
...
class CasaStockOrderLine(models.Model):
    _name = 'casa_hanane.stock.order.line'
    _description = 'Ligne de Commande Stock Casa (Hanane)'
    
    order_id = fields.Many2one('casa_hanane.stock.order', string='Commande', required=True, ondelete='cascade')
    
    stock_id = fields.Many2one('casa_hanane.stock.stock', string='Stock', required=True)
    product_id = fields.Many2one('casa_hanane.product', string='Produit', related='stock_id.product_id', store=True)
