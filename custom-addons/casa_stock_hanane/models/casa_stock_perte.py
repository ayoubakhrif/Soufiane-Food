...
class CasaStockPerteHanane(models.Model):
    _name = 'casa_hanane.stock.perte'
    _description = 'Pertes de Stock Casa Hanane'
    _order = 'date desc, id desc'
    _inherit = ['mail.thread', 'mail.activity.mixin']

    name = fields.Char(string='Référence', required=True, copy=False, readonly=True, default='/')
    stock_id = fields.Many2one('casa_hanane.stock.stock', string='Article Stock', required=True, states={'draft': [('readonly', False)]})
    
    product_id = fields.Many2one('casa_hanane.product', string='Produit', tracking=True)
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if vals.get('name', '/') == '/':
                vals['name'] = self.env['ir.sequence'].next_by_code('casa_hanane.stock.perte') or '/'
        return super(CasaStockPerteHanane, self).create(vals_list)
    ...
    def action_validate(self):
        for rec in self:
            ...
            self.env['casa_hanane.stock.move'].create(move_vals)
    ...
    def action_cancel(self):
        for rec in self:
            ...
            moves = self.env['casa_hanane.stock.move'].search([('res_model', '=', self._name), ('res_id', '=', rec.id)])
            for move in moves:
                move.copy({
                    'qty': -move.qty,
