...
class CasaStockReturn(models.Model):
    _name = 'casa_hanane.stock.return'
    _inherit = ['mail.thread', 'mail.activity.mixin']
    _description = 'Retour Client Stock Casa (Hanane)'
    _order = 'date desc, id desc'

    name = fields.Char(string='Référence', readonly=True, default='/')
    
    exit_id = fields.Many2one('casa_hanane.stock.exit', string='Sortie d\'origine', required=True, readonly=True)
    ...
    def create(self, vals):
        if vals.get('name', '/') == '/':
            vals['name'] = self.env['ir.sequence'].next_by_code('casa_hanane.stock.return') or '/'
    ...
    def action_cancel(self):
        for rec in self:
            ...
            # Reusing the helper from casa_hanane.stock.entry
            current_stock = self.env['casa_hanane.stock.entry']._get_current_stock_qty(rec)
