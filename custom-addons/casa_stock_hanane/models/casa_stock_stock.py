...
class CasaStockStock(models.Model):
    ...
    _description = 'Stock Casa (Aggregation) (Hanane)'
    _auto = False
    _log_access = False
    _order = 'quantity desc, product_id'

    product_id = fields.Many2one('casa_hanane.product', string='Produit', readonly=True)
    ...
    def action_open_dum(self):
        ...
        try:
            ...
            moves = self.env['casa_hanane.stock.move'].search([('dum', '=', self.dum)])
            moves.write({'scan_dum': file_url})
            
            # Also find entries
            entries = self.env['casa_hanane.stock.entry'].search([('dum', '=', self.dum)])
    ...
    def _compute_display_name(self):
        for rec in self:
            ...
            qty_str = f"{rec.virtual_quantity} ({(rec.virtual_quantity * weight):.2f}T)"
    ...
    def _compute_virtual_quantity(self):
        if not self.ids:
            for rec in self:
                rec.virtual_quantity = rec.quantity or 0.0
            return
            
        self.env.cr.execute("""
            SELECT col.stock_id, sum(col.qty) 
            FROM casa_hanane_stock_order_line col
            JOIN casa_hanane_stock_order co ON co.id = col.order_id
        ...
        for rec in self:
            ...
            rec.virtual_quantity = (rec.quantity or 0.0) - reserved
    ...
    def action_new_perte(self):
        ...
        return {
            ...
            'type': 'ir.actions.act_window',
            'res_model': 'casa_hanane.stock.perte',
            'view_mode': 'form',
            'target': 'current',
            'context': {
    ...
    def action_export_product_report_excel(self):
        ...
        # Gestia Branding
        sheet.write(row, 0, "Generated by Gestia ERP", workbook.add_format({'italic': True, 'font_size': 10}))
    ...
    def _get_product_report_data(self):
        ...
        for city_code in cities_to_process:
            ...
            aggregated = {}
            for rec in records:
                key = (rec.product_id.name, rec.calibre or "", rec.lot or "", rec.weight or 0.0, rec.price or 0.0)
                if key not in aggregated:
                    aggregated[key] = {'qty': 0, 'tonnage': 0, 'total': 0}
                aggregated[key]['qty'] += rec.quantity
                aggregated[key]['tonnage'] += rec.total_weight
            ...
            for (prod_name, calibre, lot, weight, price), data in aggregated.items():
