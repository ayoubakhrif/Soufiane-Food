...
class CasaStockTransfer(models.Model):
    _name = 'casa_hanane.stock.transfer'
    _description = 'Transfert de Stock Casa (Hanane)'
    ...
    # Source
    source_stock_id = fields.Many2one('casa_hanane.stock.stock', string='Stock Source', required=True, tracking=True)
    product_id = fields.Many2one('casa_hanane.product', string='Produit')
    ...
    source_ste_id = fields.Many2one('casa_hanane.ste', string='Société Source')
    ...
    dest_ste_id = fields.Many2one('casa_hanane.ste', string='Société Destination', tracking=True)
    ...
    move_out_id = fields.Many2one('casa_hanane.stock.move', string='Mouvement Sortie', readonly=True)
    move_in_id = fields.Many2one('casa_hanane.stock.move', string='Mouvement Entrée', readonly=True)
    move_cancel_out_id = fields.Many2one('casa_hanane.stock.move', string='Annulation Sortie', readonly=True)
    move_cancel_in_id = fields.Many2one('casa_hanane.stock.move', string='Annulation Entrée', readonly=True)

    @api.model
    def create(self, vals):
        if vals.get('name', '/') == '/':
            vals['name'] = self.env['ir.sequence'].next_by_code('casa_hanane.stock.transfer') or '/'
    ...
    def action_validate(self):
        for rec in self:
            ...
            move_out = self.env['casa_hanane.stock.move'].create({
                ...
                'res_model': 'casa_hanane.stock.transfer',
                'res_id': rec.id,
            })

            # ENTRY move to destination
            move_in = self.env['casa_hanane.stock.move'].create({
                'product_id': product_id,
                'lot': lot,
                'dum': dum,
