...
class CasaClientInvoiceWizard(models.TransientModel):
    _name = 'casa_hanane.client.invoice.wizard'
    _description = 'Wizard pour imprimer la facture hebdomadaire du client Casa (Hanane)'
    ...
    def _get_selection_weeks(self):
        ...
        # Sorties
        exits = env['casa_hanane.stock.exit'].sudo().search([('week', '!=', False)])
        weeks.update(exits.mapped('week'))
        
        # Avances
        avances = env['casa_hanane.client.advance'].sudo().search([('date', '!=', False)])
        for date in avances.mapped('date'):
            weeks.add(date_to_week(date))
            
        # Impayés
        impayes = env['casa_hanane.client.unpaid'].sudo().search([('date', '!=', False)])
    ...
    def action_print_invoice(self):
        ...
        return self.env.ref(
            'casa_stock_hanane.action_report_casa_client_invoice'
        ).report_action(self)
