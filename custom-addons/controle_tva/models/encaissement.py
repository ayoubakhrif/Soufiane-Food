...
class TvaEncaissement(models.Model):
    _name = 'tva.encaissement'
    _description = "Déclaration d'encaissement"
    ...
    @api.constrains('invoice_number', 'is_acompte')
    def _check_unique_invoice(self):
        for record in self:
            if record.invoice_number and not record.is_acompte:
                domain = [
                ...
                if self.search_count(domain) > 0:
                    raise ValidationError("Une facture principale avec ce numéro existe déjà.")

    @api.onchange('invoice_number', 'is_acompte')
    def _onchange_invoice_acompte(self):
        if self.invoice_number and self.is_acompte:
            domain = [
