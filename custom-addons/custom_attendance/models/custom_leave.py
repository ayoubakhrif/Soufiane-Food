...
class CustomLeave(models.Model):
    ...
    employee_id = fields.Many2one('custom.employee', string='Employé', required=True, tracking=True)
    ...
    reason = fields.Text(string='Motif / Remarques')
    
    days_count = fields.Float(string='Jours Calculés', compute='_compute_days_count', store=True, 
                              help="Nombre de jours ouvrables (exclut les jours de repos et fériés)")
    ...
    def _compute_days_count(self):
        ...
        for rec in self:
            ...
            while current_date <= rec.date_to:
                # 1. Skip Non-Working Day (e.g. Sunday)
    ...
    def action_approve(self):
        for rec in self:
            if rec.leave_type == 'paid':
                ...
                remaining = employee.leaves_remaining
                
                # Check Quota
                if rec.days_count > remaining:
                    raise exceptions.ValidationError(
                        f"Solde insuffisant ! Demandé: {rec.days_count}j, Restant: {remaining}j"
                    )
