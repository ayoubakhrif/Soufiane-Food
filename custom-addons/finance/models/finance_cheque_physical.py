...
{{
  ...
  "journal": 1
    ...
    def action_verify_cheque_ai(self):
        ...
        # Modèle Gemini configurable (défaut : gemini-flash-latest)
        gemini_model = self.env['ir.config_parameter'].sudo().get_param('finance.gemini_model', 'gemini-flash-latest')
        ...
        for rec in self:
            if not rec.doc_pdf:
                if len(self) == 1:
                    ...
                    rec.message_post(body=Markup("<div style='color:red;'>Aucun PDF de documentation n'est attaché. Vérification annulée.</div>"))
                    continue

            # Le fichier binaire est déjà stocké en base64 dans Odoo
            pdf_b64 = rec.doc_pdf.decode('utf-8') if isinstance(rec.doc_pdf, bytes) else rec.doc_pdf

            # Résoudre le nom légal complet de la société :
            societe_legale = ""
            if rec.ste_id:
                if rec.ste_id.raison_social:
                    societe_legale = rec.ste_id.raison_social
                elif rec.ste_id.core_ste_id and rec.ste_id.core_ste_id.name:
                    societe_legale = rec.ste_id.core_ste_id.name
                else:
                    societe_legale = rec.ste_id.name
