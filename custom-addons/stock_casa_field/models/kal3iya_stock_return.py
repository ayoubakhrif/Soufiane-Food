...
class Kal3iyaStockReturn(models.Model):
    _name = 'kal3iya.stock.return'
    _inherit = ['mail.thread', 'mail.activity.mixin']
    _description = 'Retour Client Kal3iya'
    _order = 'date desc, id desc'

    name = fields.Char(string='Référence', readonly=True, default='/')
    exit_id = fields.Many2one('kal3iya.stock.exit', string='Sortie Originale', required=True)
    ...
    driver_id = fields.Many2one('kal3iya.stock.driver', related='exit_id.driver_id', store=True, string='Chauffeur')
    ...
    def create(self, vals):
        if vals.get('name', '/') == '/':
            vals['name'] = self.env['ir.sequence'].next_by_code('kal3iya.stock.return') or '/'
        return super(Kal3iyaStockReturn, self).create(vals)
    ...
    def write(self, vals):
        ...
        return super(Kal3iyaStockReturn, self).write(vals)
