...
class SuiviStockEntry(models.Model):
    _name = 'suivi.stock.entry'
    _description = 'Entrée Stock Suivi Transport'
    _order = 'date desc, id desc'
    _inherit = ['mail.thread', 'mail.activity.mixin']

    name = fields.Char(string='Référence', readonly=True, default='/')
    product_id = fields.Many2one('suivi.produit', string='Produit', required=True, tracking=True)
    ...
    ville = fields.Selection([
        ('casa', 'Casa'),
    ], string='Ville', required=True, tracking=True, default='casa', readonly=True)
    ...
    fournisseur_id = fields.Many2one('suivi.fournisseur', string='Fournisseur', tracking=True)
    chauffeur_id = fields.Many2one('suivi.chauffeur', string='Chauffeur', tracking=True)
    ...
    move_id = fields.Many2one('suivi.stock.move', string='Mouvement Stock', readonly=True)
    cancel_move_id = fields.Many2one('suivi.stock.move', string='Mouvement d\'Annulation', readonly=True)
    ...
    def create(self, vals):
        if vals.get('name', '/') == '/':
            vals['name'] = self.env['ir.sequence'].next_by_code('suivi.stock.entry') or '/'
        ...
        for rec in self:
            if rec.state in ('confirmed', 'done'):
                forbidden_fields = [
                    ...
                    'date', 'lot', 'dum', 'ville', 'fournisseur_id', 'chauffeur_id'
                ]
    ...
    def action_validate(self):
        for rec in self:
            ...
            # Create Move
            move = self.env['suivi.stock.move'].create({
    ...
    def action_cancel(self):
        for rec in self:
            ...
            if rec.state == 'done':
                ...
                cancel_move = self.env['suivi.stock.move'].create({
                    'product_id': rec.product_id.id,
                    'lot': rec.lot,
                    'dum': rec.dum,
                    'ville': rec.ville,
    ...
    def _get_current_stock_qty(self, rec):
        ...
        res = self.env['suivi.stock.move'].read_group(domain, ['qty'], [])
