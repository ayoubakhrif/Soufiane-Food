...
# We need to replace the entire AI request block in _extract_empty_cheque_data
# from:
...
# I need to match the block starting from api_key = ... down to except Exception: continue
# I'll use regex to find this block and replace it.
pattern = re.compile(r"            api_key = self\.env\['ir\.config_parameter'\].sudo\(\)\.get_param\('whatsapp_stock\.openai_key'\).*?            try:\s+result = json\.loads\(raw_content\)\s+except Exception:\s+continue", re.DOTALL)

content = pattern.sub(new_ai_logic, content)

with open(filepath, 'w', encoding='utf-8') as f:
    f.write(content)
print("Updated to Gemini API")
